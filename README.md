...
- vector<Bid*> — stores bids on a listing. Bids are always placed in increasing order (enforced by the bid validation), so the highest bid is always the last element. O(1) to read the winner, O(1) amortized to add a bid.
